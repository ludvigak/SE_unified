...
if(fkg)
    pre_t = tic;
    S = se1p_precomp_kaiser(x,se_opt);
    grid_fcn = @(f) SE_fg_grid_split_kaiser_mex_1p(x(S.perm,:),f(S.perm),se_opt,...
						  S.zx,S.zy,S.zz, ...
						  S.idx);
    ...
    SI = S;
    iperm = @(u) u(SI.iperm,:);
    int_fcn = @(F) iperm(SE_fg_int_split_kaiser_mex_1p(0,F,se_opt,SI.zx, ...
                                                      SI.zy,SI.zz,SI.idx));
else
    grid_fcn = @(f) SE_fg_grid_kaiser_mex_1p(x,f,se_opt);
    int_fcn = @(f) SE_fg_int_kaiser_mex_1p(x,f,se_opt);
end
